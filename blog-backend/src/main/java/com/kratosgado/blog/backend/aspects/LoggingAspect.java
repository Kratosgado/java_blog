...
public class LoggingAspect {
  ...
  public void logBeforeController(JoinPoint joinPoint) {
    log.info(
        ">>> Entering controller method: {} with arguments: {}",
  ...
  public Object logAroundService(ProceedingJoinPoint joinPoint) throws Throwable {
    ...
    try {
      ...
      log.info("<== Completed service method: {} in {} ms", methodName, duration.toMillis());
  ...
  public Object logAroundRepository(ProceedingJoinPoint joinPoint) throws Throwable {
    ...
    try {
      ...
      log.debug("<<< Completed database operation: {} in {} ms", methodName, duration.toMillis());

      // Warn about slow queries (> 100ms)
      if (duration.toMillis() > 100) {
        log.warn(
            "!!! Slow database operation detected: {} took {} ms", methodName, duration.toMillis());
      }
      ...
      log.error(
          "<<< Failed database operation: {} after {} ms - Error: {}",
          methodName,
          duration.toMillis(),
          e.getMessage(),
          e);
  ...
  @AfterReturning(
      pointcut =
          "execution(*"
              + " com.kratosgado.blog.backend.controllers.v1.PostController.getPostBySlug(..)) &&"
              + " args(slug)",
      argNames = "slug")
