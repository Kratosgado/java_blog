...
public class TagDAO extends BaseDAO {
  ...
  public Optional<Tag> createTag(Tag tag) {
    String sql = "INSERT INTO tags (name, slug, description) VALUES (?, ?, ?) RETURNING id";
    try (Connection conn = DatabaseConfig.getConnection();
  ...
  public boolean removeAllTagsFromPost(Integer postId) {
    String sql = "DELETE FROM post_tags WHERE post_id = ?";
    try (Connection conn = DatabaseConfig.getConnection();
        PreparedStatement stmt = conn.prepareStatement(sql);) {
      stmt.setInt(1, postId);
      stmt.executeUpdate();
      logger.debug("All tags removed from post {}", postId);
      return true;
    } catch (Exception e) {
      logger.error("Failed to remove all tags from post", e);
      return false;
    }
  }
