...
public class TagRepository extends SluggableRepository<Tag> {
  ...
  public long countAll() {
    ...
    return withConnection(conn -> {
      ...
      try (PreparedStatement statement = conn.prepareStatement(query);
        ...
        throw BlogException.internal("Failed to count tags: " + e.getMessage());
  ...
  public List<TagResponse> findAllWithPostCount() {
    ...
    return withConnection(conn -> {
      ...
      List<TagResponse> tags = new java.util.ArrayList<>();
      ...
      try (PreparedStatement statement = conn.prepareStatement(query)) {
        ...
        throw BlogException.internal("Failed to find tags with post count: " + e.getMessage());
