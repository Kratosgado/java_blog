...
//
// private static final int POST_COUNT = 100_000; // Large enough to make Seq
// Scan expensive
...
//
// TimingStats legacyStats = measureSearch("Legacy LIKE", () ->
// performLegacySearch(keyword));
// TimingStats optimizedStats = measureSearch("TSVector", () ->
// performOptimizedSearch(keyword));
//
// log.info(
...
//
// double improvement = legacyStats.averageMillis() /
// optimizedStats.averageMillis();
// log.info("Performance Improvement: {}x faster", String.format("%.2f",
// improvement));
//
...
// String[] keywords = {
// "java", "spring", "performance", "database", "optimization", "index",
// "query", "search"
// };
//
...
// String currentKeyword = shouldMatch ? "java" :
...
// return postRepository.searchPublishedPostsSimple(keyword, PageRequest.of(0,
// 20));
// }
//
...
// "EXPLAIN (ANALYZE, BUFFERS, TIMING) SELECT * FROM posts WHERE status =
// 'published' AND "
...
// + "WHERE status = 'published' AND search_vector @@
...
// entityManager
// .createNativeQuery(
// "CREATE INDEX IF NOT EXISTS idx_posts_status_views ON posts(status, views
// DESC)")
// .executeUpdate();
// entityManager
// .createNativeQuery(
...
// entityManager
// .createNativeQuery(
// "CREATE INDEX IF NOT EXISTS idx_posts_category_status ON posts(category_id,
// status)")
// .executeUpdate();
// entityManager
// .createNativeQuery(
// "CREATE INDEX IF NOT EXISTS idx_posts_fts_vector ON posts USING
// GIN(search_vector)")
// .executeUpdate();
// entityManager
// .createNativeQuery(
...
// }
//
// private TimingStats measureSearch(String label, Supplier<Page<PostView>>
// search) {
