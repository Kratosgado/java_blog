...
public class HomeController {
  ...
  private List<Post> getFilteredPosts() {
    ...
    if (searchTerm.startsWith("#") && searchTerm.length() > 1) {
      ...
      try {
        ...
        logger.warn("Tag filtering not yet implemented via API");
    ...
    // Check if searching by keyword
      ...
      try {
        ...
        logger.warn("Keyword search not yet implemented via API");
        posts = getAllPostsFallback();
      }
    }
    // Default: get all published posts
      ...
      try {
        ...
        logger.warn("getPublishedPosts not yet implemented via API");
        posts = getAllPostsFallback();
      }
    }
  ...
  /**
   * Fallback method to get all posts when specific filtering is not available.
   */
