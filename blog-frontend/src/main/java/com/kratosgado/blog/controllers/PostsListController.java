...
public class PostsListController {
  ...
  @FXML private Button createNewPostBtn;
  ...
  private void setupTableColumns() {
    ...
    dateCol.setCellFactory(
        column ->
            new TableCell<Post, LocalDateTime>() {
              @Override
    ...
    viewsCol.setCellFactory(
        column ->
            new TableCell<Post, Integer>() {
              @Override
            ...
            new TableCell<Post, PostStatus>() {
              ...
              private String getStatusStyle(PostStatus status) {
                if (status == PostStatus.published) {
                  return "-fx-background-color: #d1fae5; -fx-text-fill: #065f46; -fx-padding: 4 12;"
                             + " -fx-background-radius: 12; -fx-font-size: 11px; -fx-font-weight:"
                             + " 700;";
                } else if (status == PostStatus.draft) {
                  return "-fx-background-color: #fef3c7; -fx-text-fill: #92400e; -fx-padding: 4 12;"
                             + " -fx-background-radius: 12; -fx-font-size: 11px; -fx-font-weight:"
                             + " 700;";
                } else {
                  return "-fx-background-color: #e5e7eb; -fx-text-fill: #374151; -fx-padding: 4 12;"
                             + " -fx-background-radius: 12; -fx-font-size: 11px; -fx-font-weight:"
                             + " 700;";
                }
              }
    ...
    postsTable.setRowFactory(
        tv ->
            new javafx.scene.control.TableRow<Post>() {
              @Override
              protected void updateItem(Post item, boolean empty) {
                ...
                if (empty || item == null) {
                  ...
                  // Add hover effect
                  setOnMouseEntered(
                      e -> setStyle("-fx-background-color: #f3f4f6; -fx-cursor: hand;"));
                  setOnMouseExited(
                      e -> {
                        if (getIndex() % 2 == 0) {
                          setStyle("-fx-background-color: #ffffff;");
  ...
  private void loadPosts() {
    try {
              ...
              .map(
                  pr -> {
                    ...
                    if (pr instanceof com.kratosgado.blog.dtos.response.PostResponse.PostDetails) {
                      p.setContent(
                          ((com.kratosgado.blog.dtos.response.PostResponse.PostDetails) pr)
                              .getContent());
                      p.setUpdatedAt(
                          ((com.kratosgado.blog.dtos.response.PostResponse.PostDetails) pr)
                              .getUpdatedAt());
                    ...
                    if (pr instanceof com.kratosgado.blog.dtos.response.PostResponse.WithUser) {
                      var user =
                          ((com.kratosgado.blog.dtos.response.PostResponse.WithUser) pr).getUser();
                      if (user != null) {
                    ...
                    if (pr instanceof com.kratosgado.blog.dtos.response.PostResponse.WithCategory) {
                      var category =
                          ((com.kratosgado.blog.dtos.response.PostResponse.WithCategory) pr)
                              .getCategory();
                      if (category != null) {
                        p.setCategory(
      ...
      ObservableList<Post> pageData =
          FXCollections.observableArrayList(sortedPosts.subList(startIndex, endIndex));
  ...
  private java.util.List<Post> sortPosts(java.util.List<Post> posts) {
    return switch (currentSort) {
      case "Oldest" ->
          posts.stream().sorted((a, b) -> a.getCreatedAt().compareTo(b.getCreatedAt())).toList();
      case "Most Viewed" -> posts.stream().sorted((a, b) -> b.getViews() - a.getViews()).toList();
      case "Title A-Z" ->
          posts.stream().sorted((a, b) -> a.getTitle().compareTo(b.getTitle())).toList();
      default ->
          posts.stream().sorted((a, b) -> b.getCreatedAt().compareTo(a.getCreatedAt())).toList();
    };
  }
            ...
            pageResponse.content().stream()
                .map(
                    pr -> {
                      ...
                      // Get content if PostDetails
                      if (pr
                          instanceof com.kratosgado.blog.dtos.response.PostResponse.PostDetails) {
                        p.setContent(
                            ((com.kratosgado.blog.dtos.response.PostResponse.PostDetails) pr)
                                .getContent());
                        p.setUpdatedAt(
                      ...
                      if (pr instanceof com.kratosgado.blog.dtos.response.PostResponse.WithUser) {
                        var user =
                            ((com.kratosgado.blog.dtos.response.PostResponse.WithUser) pr)
                                .getUser();
                        if (user != null) {
                          p.setUserId(user.getId());
                        }
                      ...
                      // Get category from category object if available
                      if (pr
                          instanceof com.kratosgado.blog.dtos.response.PostResponse.WithCategory) {
                        var category =
                            ((com.kratosgado.blog.dtos.response.PostResponse.WithCategory) pr)
                                .getCategory();
                        if (category != null) {
